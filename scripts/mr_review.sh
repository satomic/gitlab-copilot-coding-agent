...
# Retry logic for Copilot code review
MAX_RETRIES=3
RETRY_DELAY=10
RETRY_COUNT=0
SUCCESS=false
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
  ...
  if timeout 3600 copilot -p "$REVIEW_PROMPT" --allow-all-tools > review_raw.txt 2>&1; then
    ...
    if [ $EXIT_CODE -eq 124 ]; then
      ...
      # Check if review_findings.json or review_summary.txt was created despite the error
      if [ -f review_findings.json ] || [ -f review_summary.txt ]; then
        echo "[INFO] Found review output despite error, attempting to continue..." >&2
        SUCCESS=true
        break
      fi
...
if [ "$SUCCESS" = false ]; then
  echo "[ERROR] Failed to complete code review after $MAX_RETRIES attempts" >&2
  exit 1
fi
